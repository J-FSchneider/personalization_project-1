...
def popularity_cat(df, target, column):
    """
    This function generates the categorical columns for the artist's popularity features
    by first generating categories based on certain thresholds and then pivoting
    them into new columns
    :param df: Dreezer's database in the DataFrame format
    :param target: the name of the identifier columns
    :param column: the name of the column where the threshold is going to be applied
    :return df: a DataFrame with the artist's identifier and the category columns
    """
    n = len(df)
    cat1 = "unpop"
    cat2 = "known"
...
def art_hits(df, target, column, threshold=0.75):
    """
    This function calculates the number of  songs that generate a certain percentage of the
    total number of songs that the artist was played upon.
    :param df: Dreezer's database in the DataFrame format
    :param target: the name of the identifier columns
